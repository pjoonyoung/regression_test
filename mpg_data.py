...
x = mpg_df1.drop(['mpg'],axis=1) # mpg를 제외 한 독립변수들

X_train,X_test, Y_train,Y_test = train_test_split(x, y, test_size=0.3, random_state=0)
# 훈련용 데이터와 평가용 데이터 분할

lr = LinearRegression() # 회귀모델생성
lr.fit(X_train,Y_train) # 모데레 훈련
...
mpg_inteercept = lr.intercept_
mpg_coef = lr.coef_

print('절편:',mpg_inteercept)
print('회귀계수:',mpg_coef)
...
#
# plt.show()
print('연비를 예측하고싶은 자동차의 정보를 입력하세요.')

cylinders_ = input('실린더 수를 입력하세요:')
